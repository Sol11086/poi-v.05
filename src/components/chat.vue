        ...
        <div class="chat-area flex-1 flex flex-col bg-gray-700">
            <div v-if="selectedChat.id || (selectedChat.roomType === 'private' && selectedChat.recipientId)"
                ...
                <template v-if="selectedChat.roomType === 'private' && selectedChat.recipientId">
                    <Avatar v-if="selectedChat.avatar" :image="selectedChat.avatar || '/default_avatar.png'"
                        class="mr-3 cursor-pointer" @click="showChatHeaderContactPopover($event)" aria-haspopup="true"
                        aria-controls="chatHeaderContactPopover" v-tooltip.bottom="'View Profile'" />
                    <h2 class="text-xl cursor-pointer hover:underline" @click="showChatHeaderContactPopover($event)"
                        aria-haspopup="true" aria-controls="chatHeaderContactPopover">
                        {{ selectedChat.name }}
                    </h2>
                </template>
                <template v-else>
                    <Avatar :image="selectedChat.avatar || '/default_team_avatar.png'" class="mr-3" />
                    <h2 class="text-xl">{{ selectedChat.name }}</h2>
                </template>
            </div>
            ...
            <Popover ref="chatHeaderContactPopoverRef" id="chatHeaderContactPopover">
                <div v-if="activeChatContactDetails && activeChatContactDetails.isLoading" class="p-2">Loading contact
                    info...</div>
                <div v-else-if="activeChatContactDetails && activeChatContactDetails.error" class="p-2 text-red-500">{{
                    activeChatContactDetails.error }}</div>
                <div v-else-if="activeChatContactDetails && activeChatContactDetails.id" class="p-3 min-w-[200px]">
                    <img :src="activeChatContactDetails.avatar || '/default_avatar.png'" alt="avatar"
                        class="w-16 h-16 rounded-full mx-auto mb-2 object-cover" />
                    <div class="text-center">
                        ...
                        <div class="text-xs mt-1">Status: <span
                                :class="activeChatContactDetails.status === 'online' ? 'text-green-400' : 'text-gray-400'">{{
                                activeChatContactDetails.status }}</span></div>
